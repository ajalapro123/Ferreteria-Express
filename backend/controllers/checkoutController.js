    ...
    try {
      ...
      try {
        ...
        for (const it of items) {
          const [rows] = await conn.query('SELECT precio, cantidad FROM productos WHERE id = ? FOR UPDATE', [it.id]);
        ...
        for (const it of items) {
          const [pRows] = await conn.query('SELECT precio FROM productos WHERE id = ?', [it.id]);
          const precio_unitario = pRows[0].precio;
        ...
        await conn.commit();
        return res.json({ ok: true, venta: { id: ventaId, total } });
      } catch (err) {
        await conn.rollback();
